...
function queueWebhookLog(type,message){
  ...
  if(
    String(message).startsWith("[CHAT]")
  ){
    return;
  }
  ...
  webhookQueue=webhookQueue.then(async()=>{
    try{
      ...
      if(!response.ok){
        ...
        originalError(
          `[WEBHOOK] Discord respondió ${response.status}: ${text}`
        );
      }
    }catch(e){
      originalError(
...
async function sendLog(
  ...
  if(!DISCORD_LOG_WEBHOOK){
    originalError(
      "[WEBHOOK] DISCORD_LOG_WEBHOOK no configurado"
    );
...
function recordError(provider,error){
  ...
  if(
    debugState.providerErrors[provider].length>10
  ){
...
async function saveMemory(){
  if(!GITHUB_TOKEN){
    console.warn(
      "[MEMORY] No se puede guardar: falta GITHUB_TOKEN."
    );
  ...
  server.listen(
    ...
    async()=>{
      ...
      if(DISCORD_LOG_WEBHOOK){
        ...
        console.warn(
          "[WEBHOOK] Logs: INACTIVOS — falta DISCORD_LOG_WEBHOOK"
        );
      }
    }
  );
