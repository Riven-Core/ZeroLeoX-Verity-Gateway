...
function remember(id,text){
  const u=user(id);
  u.memory.push(text);
  if(u.memory.length>50)u.memory.shift();
  save();
}

function forget(id,text){
  const u=user(id);
  u.memory=u.memory.filter(x=>x.toLowerCase()!==text.toLowerCase());
  save();
}
...
function memoryText(id){
  const u=user(id);
  return u.memory.length?u.memory.join("\n"):"Sin recuerdos guardados.";
}

function identity(id){
  if(id===CREATOR)return "Este usuario es ZeroLeoX, el creador de Riven.";
  if(id===DEV2)return "Este usuario es el segundo developer de Riven.";
    ...
    await fetch(WEBHOOK,{
      ...
      body:JSON.stringify({
        ...
        embeds:[{
          ...
          color,
          footer:{text:"Riven • Gateway Logs"},
...
async function callAI(url,headers,body,label){
...
async function gemini(id,name,msg){
  if(!GKEY)throw new Error("Gemini API key no configurada");
...
async function groq(id,name,msg){
  if(!QKEY)throw new Error("Groq API key no configurada");
...
async function openrouter(id,name,msg){
  if(!RKEY)throw new Error("OpenRouter API key no configurada");
...
async function mistral(id,name,msg){
  if(!MKEY)throw new Error("Mistral API key no configurada");
...
async function ask(id,name,msg){
  ...
  for(const [nameProvider,fn] of providers){
    try{
      ...
      errors.push(`${nameProvider}: ${e.message}`);

      lastError=e.message;
      lastErrorAt=new Date().toISOString();

      console.log(`⚠️ ${nameProvider} falló:`,e.message);

      if(nameProvider==="Mistral"&&e.message==="TIMEOUT"){
  ...
  throw new Error(
    errors.length
      ? errors.join(" | ")
      : "Todos los proveedores devolvieron una respuesta vacía."
  );
}
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(/^recuerda\s+/i.test(message)){
      const memory=message.replace(/^recuerda\s+/i,"").trim();
    ...
    if(/^olvida\s+/i.test(message)){
      const memory=message.replace(/^olvida\s+/i,"").trim();
    ...
    const reply=await ask(userId,name,message);
