...
function recordError(
  ...
  if(
    debugState.lastErrors.length>20
  ){
...
async function loadMemory(){
  if(!GITHUB_TOKEN){
    console.warn(
      "[MEMORY] GITHUB_TOKEN no configurado."
    );
  ...
  try{
    ...
    console.log(
      "[MEMORY] memory.json cargado desde GitHub."
    );
  }catch(e){
    console.error(
...
async function handleMemoryCommand(
  ...
  if(
    ...
    lower.startsWith("acuérdate de ")
  ){
    content=
      original.slice(13).trim();

    action="recordar";
  }else if(
    lower.startsWith("acuerdate de ")
  ){
    content=
      original.slice(13).trim();
    ...
    content=
      original.slice(7).trim();

    action="olvidar";
  }else if(
    lower.startsWith("olvídate de ")
  ){
    content=
      original.slice(12).trim();

    action="olvidar";
  }else if(
    lower.startsWith("olvidate de ")
  ){
    content=
      original.slice(12).trim();
  ...
  if(action==="recordar"){
    ...
    if(!added){
      return(
        "🧠 Ya tenía ese recuerdo guardado."
      );
    }
