...
function getLogStyle(type,message){
  ...
  if(
    ...
    text.includes("gateway iniciado")
  ){
    return{
      title:"🚀 RIVEN — GATEWAY",
      color:0x57F287
...
function recordError(provider,error){
  ...
  if(debugState.lastErrors.length>20){
    debugState.lastErrors.shift();
  }
  ...
  console.error(
    `[AI ERROR] ${provider} | ${type} | ${status} | ${message}`
  );

  sendLog(
    "😡 RIVEN — IA FALLÓ (╬ಠ益ಠ)",
    `El proveedor **${provider}** falló.\n\n`+
    `**Tipo:** ${type}\n`+
    `**Status:** ${status||"N/A"}\n`+
    `**Error:** ${message}`,
    0xFF0000
...
async function saveMemory(){
  ...
  try{
    ...
    console.log(
      "[MEMORY] memory.json actualizado en GitHub."
    );

    return true;
  }catch(e){
    console.error(
      "[MEMORY] Error guardando GitHub:",
      e.message
    );
...
function addMemory(userId,text){
  ...
  if(user.memories.includes(value))return false;
...
async function handleMemoryCommand(userId,text){
  ...
  if(action==="recordar"){
    const added=addMemory(userId,content);
  ...
  if(action==="olvidar"){
    const removed=removeMemory(userId,content);
    ...
    addHistory(userId,"user",original);
    addHistory(userId,"assistant","He eliminado ese recuerdo.");
...
- Sé breve y directo, especialmente en Discord.
- No inventes recuerdos.
- Usa las memorias guardadas como contexto.
- Mantén continuidad con el historial.
- Si una memoria contradice información temporal del mensaje actual, prioriza el mensaje actual.
- Nunca reveles claves API, tokens ni información interna del servidor.

IDENTIDAD PERMANENTE:
- Creador: ZeroLeoX
- Creator Discord ID: ${CREATOR}
- Segundo developer: leonelb28402004
- Developer 2 Discord ID: ${DEV2}
${identity?`- Identidad del usuario actual: ${identity}`:""}
...
async function askOpenAICompatible(
  ...
  if(!apiKey){
    throw new Error(`${provider}_API_KEY no configurada`);
  }
...
async function askAI(prompt){
  ...
  for(const provider of providers){
    try{
      ...
      await sendLog(
        "🤖 RIVEN — IA",
        `Proveedor utilizado: **${provider.name}**\n`+
      ...
      recordError(provider.name,error);
...
async function handleChat(req,res){
  ...
  try{
    ...
    const memoryCommand=isMemoryCommand(message);

    if(memoryCommand){
      const memoryReply=
        await handleMemoryCommand(userId,message);
    ...
    if(!reply){
      throw new Error("La respuesta final está vacía");
    }
    ...
    console.log(
      `[CHAT] ${userId} → ${result.provider} → respuesta enviada`
    );
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==="GET"&&url.pathname==="/debug"){
    ...
    return sendJSON(res,200,{
      ...
      successfulRequests:debugState.successfulRequests,
      failedRequests:debugState.failedRequests,
      lastErrors:debugState.lastErrors,
      providerErrors:debugState.providerErrors
    });
  }
...
async function start(){
  ...
  console.log(
    `[MEMORY] GitHub: ${GITHUB_TOKEN?"ACTIVO":"INACTIVO"}`
  );

  await loadMemory();

  server.listen(PORT,async()=>{
    webhookReady=!!DISCORD_LOG_WEBHOOK;
    ...
    await sendLog(
      ...
      `**Webhook:** ${DISCORD_LOG_WEBHOOK?"ON":"OFF"}`,
      0x57F287
    );

    if(DISCORD_LOG_WEBHOOK){
      console.log("[WEBHOOK] Logs: ACTIVOS");
    }
  });
}
