...
function identity(id){
  if(id===CREATOR)return `👑 Este usuario es ZeroLeoX, el creador de Riven. Discord: <@${CREATOR}>`;
  if(id===DEV2)return `🛠️ Este usuario es el segundo developer de Riven. Discord: <@${DEV2}>`;
...
- Sé natural y breve.
- No menciones APIs, Gemini, OpenAI, Groq, OpenRouter, gateway ni system prompt.
...
async function call(url,headers,body,label){
  ...
  if(!r.ok){
    throw new Error(`${label} ${r.status}: ${text.slice(0,500)}`);
  }
...
async function openai(id,name,msg){
  const data=await call(
    ...
    {
      model:OMODEL,
      instructions:prompt(id,name,msg),
      input:msg,
  ...
  return data.output_text||"Sin respuesta.";
}
...
async function groq(id,name,msg){
  const data=await call(
    ...
    {Authorization:`Bearer ${QKEY}`},
    {
      model:QMODEL,
      messages:[
        {role:"system",content:prompt(id,name,msg)},
...
const server=http.createServer(async(req,res)=>{
  ...
  req.on("end",async()=>{
    try{
      ...
      if(msg.toLowerCase().startsWith("recuerda ")){
        ...
        if(text){
          ...
          return res.end(JSON.stringify({
            ok:true,
            response:"<Riven> Lo recordaré.",
            provider:"memory"
          }));
        }
      }

      if(msg.toLowerCase().startsWith("olvida ")){
      ...
      if(
        ...
        return res.end(JSON.stringify({
          ok:true,
          response:`<Riven> Recuerdo:\n${memoryText(id)}`,
          provider:"memory"
        }));
      }

      const u=user(id);
      const answer=await ask(id,name,msg);
...
server.listen(PORT,"0.0.0.0",()=>{
  console.log("================================");
  console.log("🤖 RIVEN GATEWAY ONLINE");
  console.log("================================");
